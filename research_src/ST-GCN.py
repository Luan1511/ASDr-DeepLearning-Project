...
class Kinect2DNormalizedDataset(Dataset):
    ...
    def __init__(self,
        ...
        # Nếu remove_head_neck, cập nhật indices (vì Head và Neck bị bỏ)
...
class Graph:
    ...
    - 'kinect23': 23 joints (loại bỏ Head và Neck)
    ...
    def _build_adjacency_matrix_23joints(self):
        """
        Xây dựng adjacency matrix cho 23 joints (không có Head và Neck).

        Joints mapping sau khi loại bỏ Head(0) và Neck(1):
        - SpineShoulder: 0 (was 2)
        - ShoulderLeft: 1 (was 3)
        - ShoulderRight: 2 (was 4)
        - ElbowLeft: 3 (was 5)
        - ElbowRight: 4 (was 6)
        - WristLeft: 5 (was 7)
        - WristRight: 6 (was 8)
        ...
        - HandLeft: 9 (was 11)
        - HandRight: 10 (was 12)
        - HandTipLeft: 11 (was 13)
        - HandTipRight: 12 (was 14)
        - SpineMid: 13 (was 15)
        - SpineBase: 14 (was 16)
        - HipLeft: 15 (was 17)
        - HipRight: 16 (was 18)
        ...
        neighbor_link = [
            # Spine connections
            (0, 13),  # SpineShoulder -> SpineMid
            (13, 14),  # SpineMid -> SpineBase

            # Left arm
            (0, 1),  # SpineShoulder -> ShoulderLeft
            (1, 3),  # ShoulderLeft -> ElbowLeft
            (3, 5),  # ElbowLeft -> WristLeft
            (5, 7),  # WristLeft -> ThumbLeft
            (5, 9),  # WristLeft -> HandLeft
            (9, 11),  # HandLeft -> HandTipLeft

            # Right arm
            (0, 2),  # SpineShoulder -> ShoulderRight
            (2, 4),  # ShoulderRight -> ElbowRight
            (4, 6),  # ElbowRight -> WristRight
            (6, 8),  # WristRight -> ThumbRight
            (6, 10),  # WristRight -> HandRight
            (10, 12),  # HandRight -> HandTipRight

            # Left leg
            (14, 15),  # SpineBase -> HipLeft
            (15, 17),  # HipLeft -> KneeLeft
            (17, 19),  # KneeLeft -> AnkleLeft
            (19, 21),  # AnkleLeft -> FootLeft

            # Right leg
            (14, 16),  # SpineBase -> HipRight
            (16, 18),  # HipRight -> KneeRight
            (18, 20),  # KneeRight -> AnkleRight
            (20, 22),  # AnkleRight -> FootRight
        ]
...
class ST_GCN(nn.Module):
    ...
    Hỗ trợ cả 25 joints (openpose25) và 23 joints (kinect23).
    """
...
class Trainer:
    ...
    def __init__(self, config: Config):
        ...
        self.num_joints = 23 if config.remove_head_neck else 25
    ...
    def build_model(self):
        ...
        if self.num_joints == 23:
            print("✓ Creating graph for 23 joints (no Head & Neck)")
    ...
    def plot_training_curves(self):
        ...
        # Summary
        axes[1, 1].axis('off')
        joints_info = f"{self.num_joints} joints" + (" (no Head & Neck)" if self.config.remove_head_neck else "")
        dynamics_info = ""
        if self.config.include_dynamics:
...
def parse_args():
    ...
    parser.add_argument('--remove-head-neck', action='store_true',
                        help='Remove Head and Neck keypoints (25 -> 23 joints)')
...
def main():
    ...
    print(f"Remove Head & Neck: {config.remove_head_neck}")
    print(f"Expected joints: {23 if config.remove_head_neck else 25}")
